...
**Objective:** Prepared the on-premises environment for cloud synchronization by aligning UPN suffixes and performing data hygiene audits.

### 1.1 Domain Verification
Before syncing local identities, I verified the domain in the Microsoft Entra admin center to ensure it is ready to accept synchronized accounts.
...
## ☁️ Phase 2: Hybrid Identity Synchronization

**Objective:** Established a hybrid identity bridge by deploying Microsoft Entra Connect to synchronize on-premises Active Directory objects with the Microsoft Entra cloud tenant.

### 2.1 Synchronization Deployment
I chose to deploy the traditional **Microsoft Entra Connect Sync** (Connect Sync) to establish a secure synchronization bridge between the on-premises environment and the cloud. 

*Rationale:* While Microsoft recommends Cloud Sync for modern, lightweight deployments, I selected the classic Connect Sync for this lab because it remains the industry standard for complex hybrid environments. Mastering this agent provides deeper insight into synchronization rules, attribute mapping, and the core mechanics of identity propagation.

**Architectural Note:** 
While Microsoft best practices recommend isolating the Entra Connect Sync service on a dedicated domain-joined member server to ensure separation of functions and resource security, this lab environment consolidates the sync agent onto `DC01` to optimize for resource efficiency. This design choice prioritizes minimizing the hardware footprint while maintaining full functional parity with an enterprise-level hybrid identity deployment.

Configuration Note: > "I chose the 'Customize' installation path instead of Express settings. This was necessary to correctly handle the non-routable .local Active Directory forest and ensure that UPN mapping is explicitly configured to align with my routable cloud domain, preventing authentication conflicts."
...
I configured the environment to use **Password Hash Synchronization** combined with **Single Sign-On (SSO)**. 
...
I authenticated the synchronization agent against my Microsoft Entra ID tenant using a Global Administrator account. This established the trusted relationship required for the agent to securely propagate identity data from the on-premises domain to the cloud.
<p align="center">
  <img src="https://i.imgur.com/4NJ3ti8.png" alt="Image Description" width="700">
</p>

### 2.5 Directory Connection & Synchronization Configuration

I connected the Microsoft Entra Connect service to my on-premises `LAB.local` forest. By creating a dedicated synchronization account with appropriate directory permissions, I established the necessary bridge to allow user and identity attributes to be read from my local Active Directory for subsequent cloud synchronization.

> **Configuration Note:** I configured the Entra Connect wizard to "Continue without matching all UPN suffixes," intentionally ignoring the non-routable `lab.local` suffix. This action allowed the synchronization service to proceed, leveraging the previously updated routable `frankminaitlab.onmicrosoft.com` UPNs for all synchronized identities.
...
I opted to "Sync all domains and OUs" for the initial configuration. This ensures that the entire directory structure is replicated to Microsoft Entra ID, providing a complete view of the lab environment's identity objects in the cloud. This approach maximizes visibility while I explore the synchronization process.
<p align="center">
...
I configured the synchronization engine to use "Azure-managed source anchors" to link my local users to the cloud.
...
I configured the sync engine to "Synchronize all users and devices." This ensures my entire lab directory is replicated to the cloud, allowing for a comprehensive and low-maintenance test environment.
<p align="center">
  <img src="https://i.imgur.com/hGO3cfi.png" alt="Image Description" width="700">
</p>

### 2.9 Optional Features
I maintained the default selection of "Password hash synchronization." This is a fundamental component of the hybrid identity model, ensuring that user credentials can be validated in Microsoft Entra ID without requiring additional infrastructure such as a full Exchange hybrid deployment.
<p align="center">
...
I reviewed the final summary to ensure all settings—specifically Single Sign-On and password synchronization were configured correctly.
...
Everything looked good, so I selected "Install" to complete the setup and have the synchronization process running.
|  |  |
| :--- | :--- |
...
* **Cloud Sync Check:** Verified in the Microsoft Entra admin center that on-premises user objects are successfully synchronizing.
...
* **Hybrid Join:** Capture the output of dsregcmd /status from one of your Windows 11 workstations to demonstrate it has successfully performed a "Hybrid Azure AD Join".
* **SSO Validation:** Document a successful login to myapps.microsoft.com using the on-premises credentials of a synced user.  
...
-Encountered an 'Unsupported browser' error during the Microsoft Entra Connect sign-in phase. Resolved this by [choose your method: e.g., configuring Edge as the system default/disabling ESC temporarily]. This highlighted the importance of browser-engine compatibility when running cloud-integrated installers on hardened Windows Server environments.
